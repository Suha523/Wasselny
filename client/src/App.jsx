...
function App() {
  ...
  const postAddvertise = (addvertiseData) => {
    axios
      .post("http://localhost:4000/addvertises", addvertiseData)
      .then((response) => {
        let addvertise = response.data;
        setAddvertises((prevState) => [...prevState, addvertise]);
        getAddvertises();
      });
  };
      ...
      <BrowserRouter>
        ...
        <div className='d-flex justify-content-between m-5'>
          <Routes>
            ...
            <Route
              path='/addAddvForm'
              element={<AddAddvForm postAddvertise={postAddvertise} />}
            />
          </Routes>
        </div>
      </BrowserRouter>
